...
#define G134_PRECISION 0.05
    ...
    case 206:
        Com::printFLN(PSTR("LDR Value: "),analogRead(A3));
        break;

    case 298:
        GCode vibrateON_A;
        GCode vibrateOFF_A;
        vibrateON_A.parseAscii("M106 P0 S255 I0",  false);
        vibrateOFF_A.parseAscii("M106 P0 S0 I0",false);

        executeGCode(&vibrateON_A);
        delay(2000);
        executeGCode(&vibrateOFF_A);
        break;
    case 299:
        pinMode(4, INPUT);
        for(int i=0;i<200;i++){
            Com::printFLN(PSTR("x: "),digitalRead(4));
            delay(5);
        }
        break;
    ...
    case 207:
        if(com->hasX() && com->hasY()){
            Com::printFLN(PSTR("X: "),com->X);
            Com::printFLN(PSTR("Y: "),com->Y);
        } 
        break; 

    case 223:
        Com::printFLN(PSTR("LONG OPEN"));
        pinMode(46, OUTPUT);
        ...
        break;
    case 224:
        Com::printFLN(PSTR("sorter shake LONG"));
        pinMode(46, OUTPUT);
        digitalWrite(46,HIGH);
        delay2(425);
        digitalWrite(46,LOW);
        ...
        break;
    case 225:
        Com::printFLN(PSTR("sorter shake"));
        pinMode(46, OUTPUT);
        digitalWrite(46,HIGH);
        delay2(225);
        digitalWrite(46,LOW);
    ...
    case 227:
        ...
        delay2(150);
        shorted = digitalRead(4);
        if(shorted == 1){ //connected, need to raise
            moveDown = false;
            supermoveZ(false,0);
            delay2(150);
        }
        ...
        for(int j=0;j<200;j++){
            // delay2(3);
            // do {
            //     shorted = analogRead(A4);
            // } while(shorted > HIGH_DETECT);
            // delay2(10);
            // do {
            //     shorted2 = analogRead(A4);
            // } while(shorted2 > HIGH_DETECT);

            // if(shorted2-shorted > 200 || shorted2-shorted < -200 ){
            //     continue;
            // }
            //software filter
            delay2(3);
            for(int k=0;k<20;k++){
                shorted = shorted + digitalRead(4);
            }
            shorted = shorted/20.0;
            if (shorted > .5f){
                shorted = 1;
            }
            else{
            ...
            //Com::printFLN(PSTR("a: "),shorted);
            if(shorted == 0 && moveDown){ //still not connected
                moveZ(moveDown,0);
            }
            else if(!moveDown){ 
                if(shorted == 1){ //still touching plate, keep moving up
                    moveZ(moveDown,0);
                }
                else{ // now move down
                    j = 40;
            ...
            else{ // must be at edge
                Com::printFLN(PSTR("HIT GND "));
                for(int i=0; i<2;i++){
                    moveZ(false,0);
                }
                break;
            ...
            if(j==199){
                Com::printFLN(PSTR("INTERFACE NOT DETECTED"));
            }
        }
        ...
        //check for and pass bead when detected
    ...
    // case 209:
    //     //run when bead should have dropped and be detected - if not found, try a few things before throwing an error and stopping 

    //     Com::printFLN(PSTR("CALLING 209..."));


    //     int vib_patterns = 6;
    //     GCode vibrateCodes[vib_patterns];

    //     vibrateCodes[0].parseAscii("M106 P0 S90 I0",  false);
    //     vibrateCodes[1].parseAscii("M106 P0 S120 I0", false);
    //     vibrateCodes[2].parseAscii("M106 P0 S140 I0", false);
    //     vibrateCodes[3].parseAscii("M106 P0 S110 I0", false);
    //     vibrateCodes[4].parseAscii("M106 P0 S70 I0", false);
    //     vibrateCodes[5].parseAscii("M106 P0 S0 I0",false);


    //     int MACHINE_TOO_HIGH = 300; //too bright after this because bead covers less space
    //     int MACHINE_TOO_LOW = 650; //too dark after this because bead covers too much space
    //     int analogVal = 0;

    //     Com::printFLN(PSTR("CALLING 209-2..."));

    //     for(int j=0;j<vib_patterns;j++){
    //         executeGCode(&vibrateCodes[j]);
    //         for(int i=0;i<24;i++){
    //             delay2(50);
    //             do {
    //                 analogVal = analogRead(A3);
    //             } while(analogVal > HIGH_DETECT);
    //             if (analogVal > LOW_DETECT)
    //                 break;
    //         }
    //         if (analogVal > LOW_DETECT)
    //             break;
    //     }
    //     //turn off vibration
    //     executeGCode(&vibrateCodes[vib_patterns-1]);
    //     Com::printFLN(PSTR("analog read : "),analogVal);
    //     const int BUF_MAX = 30;
    //     char buf[BUF_MAX];
    ...
    //     GCode up;
    //     if (analogVal > LOW_DETECT){
    //         Com::printFLN(PSTR("BEAD DETECTED!!!!"));
            
    //         if (analogVal < MACHINE_TOO_HIGH ){
    //             newPos = -.1f;
    //             strcpy_P(buf, (const char*) F("G1 Z"));
    //             dtostrf(newPos, 8, 6, val);
    //             strcat(buf, val);
    //             strcat_P(buf, (const char*) F(" F1000"));
    //             Com::printFLN(PSTR("---DOWN--- Z To: "),Printer::currentPosition[2] + newPos,4);
    //             Com::printFLN(PSTR("AVAL : "),analogVal);
    //             up.parseAscii(buf,false);
    //             executeGCode(&up);
    //         }
    //         if (analogVal > MACHINE_TOO_LOW){
    //             for(int j=0;j<3;j++){
    //                 newPos = .1f;
    //                 strcpy_P(buf, (const char*) F("G1 Z"));
    //                 dtostrf(newPos, 8, 6, val);
    //                 strcat(buf, val);
    //                 strcat_P(buf, (const char*) F(" F1000"));
    //                 Com::printFLN(PSTR("---UP--- Z To: "),Printer::currentPosition[2] + newPos,4);
    //                 Com::printFLN(PSTR("AVAL : "),analogVal);
    //                 up.parseAscii(buf,false);
    //                 executeGCode(&up);
    //                 delay2(50);
    ...
    //         break;
    //     }
    //     Com::printFLN(PSTR("NO BEAD DETECTED :( "));

    //     //first try and lower machine a bit
    //     for(int j=0;j<10;j++){
    //         newPos = -.1f;
    //         strcpy_P(buf, (const char*) F("G1 Z"));
    //         dtostrf(newPos, 8, 6, val);
    ...
    //     }
    //     if (analogVal > LOW_DETECT){
    //         Com::printFLN(PSTR("ADDITIONAL LOWER SUCCESS"));
    //         break;
    //     }
    //     else{
    //         newPos = 1.0f;
    ...
    //         executeGCode(&up);
    //     }
    //     //try to vibrate sorter, then vibrate chooser
    //     pinMode(46, OUTPUT);
    //     digitalWrite(46,HIGH);
    //     delay2(250);
    //     digitalWrite(46,LOW);
    //     delay2(3000);
    //     executeGCode(&vibrateCodes[1]);
    //     delay2(500);
    //     executeGCode(&vibrateCodes[vib_patterns-1]);
    //     while(1){
    //         do {
...
void Commands:: G303(){
    ...
    vibrateON.parseAscii("M106 P0 S255 I0",  false);
    vibrateON2.parseAscii("M106 P0 S140 I0",  false);
    vibrateOFF.parseAscii("M106 P0 S0 I0",false);
    int jj=0;
    for(jj=0;jj<2;jj++){
        if(jj==1 && foundd == false){
                Com::printFLN(PSTR("try"));
                executeGCode(&vibrateOFF);
                delay2(50);
                moveMotor(0, 0,800);
                foundd = CheckForBead(500);
                if(foundd){
                    break;
                }
                executeGCode(&vibrateON);
                foundd = CheckForBead(2000);
                if(foundd){
                    break;
                }
                executeGCode(&vibrateOFF);
        ...
        foundd = CheckForBead(800);
        if(!foundd){
            executeGCode(&vibrateON2);
            foundd = CheckForBead(800);
            if(!foundd){
                executeGCode(&vibrateON);
                int i = 0;
    ...
    if(!foundd){
        ...
        while(1){
            GCode::keepAlive(Processing);
            Commands::checkForPeriodicalActions(true);
            if(CheckForBead(2000)){ 
                Com::printFLN(PSTR("hey"));
                break;
            }   
        }
    }
...
void Commands:: supermoveZ(boolean moveDown,int speed){
    const int BUF_MAX = 30;
    char buf[BUF_MAX];
    const int VAL_MAX = 16;
    char val[VAL_MAX];
    ...
    dtostrf(newPos, 8, 6, val);
    strcat(buf, val);
    strcat_P(buf, (const char*) F(" F3000"));
    if (moveDown){
        Com::printFLN(PSTR("-DOWN- "),Printer::currentPosition[2] + newPos,4);
    }
    else{
        Com::printFLN(PSTR("-UP- "),Printer::currentPosition[2] + newPos,4);
    }
    move.parseAscii(buf,false);
    executeGCode(&move);
}

void Commands:: moveZ(boolean moveDown,int speed){
    const int BUF_MAX = 30;
    char buf[BUF_MAX];
    const int VAL_MAX = 16;
    char val[VAL_MAX];
...
void Commands:: servoPass(int angle){
    GCode move_pusher;
    move_pusher.parseAscii("G205 P0 S0 E0 F800",  false);
    executeGCode(&move_pusher);
    delay(50);

}
void Commands:: delay2(uint32_t delay){
    delay += HAL::timeInMilliseconds();  // keep track of when we started waiting
    while((uint32_t)(delay - HAL::timeInMilliseconds())  < 2000000000 ) {
        GCode::keepAlive(Processing);
        Commands::checkForPeriodicalActions(true);
    }
}
...
int analogVall = 0;
int HIGH_DETECTT = 900;
int LOW_DETECTT = 320;
uint32_t startTimee;
uint32_t endTimee;
boolean Commands:: CheckForBead(uint32_t checkTime){

    startTimee = HAL::timeInMilliseconds();
    endTimee = startTimee + checkTime;

    while(HAL::timeInMilliseconds()<endTimee){
        if (digitalRead(A3) == 1){
            //if (analogVall < HIGH_DETECTT){
                Com::printFLN(PSTR("DETECTED!"));
                return true;
            //}
        }
    }
    Com::printFLN(PSTR("NADA"));
    return false;
}
...
void Commands::processMCode(GCode *com) {
	...
	case 18: // M18 is to disable named axis
		...
		break;
    case 19: // M18 is to disable named axis
        Commands::waitUntilEndOfAllMoves();
        if(com->hasX()) {
