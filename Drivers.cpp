...
void commandG201(GCode &code)
{
    ...
    if(!code.hasX()) return;
    motorDrivers[id]->gotoPosition(code.X, speed);
}
...
void commandG205(GCode &code)
{
	...
	if(id < 0) id = 0;
	if(id >= NUM_MOTOR_DRIVERS) id = 0;
	motorDrivers[id]->home(code.hasS() && code.S != 0, code.hasE() && code.E != 0,speed);
}
void moveMotor(int id, int pos,int speed)
{
    motorDrivers[id]->gotoPosition(pos, speed);
}
...
void enableAllMotorDrivers()
{
    for(int i = 0; i < NUM_MOTOR_DRIVERS; i++)
        motorDrivers[i]->enable();
}
