...
public:
    ...
    virtual void gotoPosition(float newPos,float temp_speed) = 0;
    virtual void enable() = 0;
    virtual void disable() = 0;
	virtual void home(bool goToCurrent, bool onlyIfNotHomed,float speed) = 0;
};
...
public:
    ...
    void gotoPosition(float newPos,float temp_speed)
    {
        enable();
	...
	void home(bool goToCurrent, bool onlyIfNotHomed,float speed) {}
};
...
class StepperDriverWithEndstop : public MotorDriverInterface
{
	...
	void gotoPosition(float newPos,float temp_speed)
	{
		int32_t temp_delay = 500000 / (temp_speed * stepsPerMM);
		bool up = true;
		if(newPos < 0)
			newPos = 0;
		...
		while(target) {
			HAL::digitalWrite(stepPin, HIGH);
			HAL::delayMicroseconds(temp_delay);
			HAL::digitalWrite(stepPin, LOW);
			HAL::delayMicroseconds(temp_delay);
			target--;
			HAL::pingWatchdog();
	...
	void home(bool goToCurrent, bool onlyIfNotHomed,float speed) {
		...
		int pusher = 0;
		if(minEndstop) {
			if(dirPin == ORIG_E0_DIR_PIN){
				Com::printFLN(PSTR("home pusher"));
				pusher = 1;
			}
			else{
				Com::printFLN(PSTR("home sorter"));
			}
			setCurrentAs(maxDistance);
			gotoPosition(0,speed);
			if(!endstopHit() && pusher){
				home(false,false,speed);
			}
			else if(!endstopHit()){
				Com::printErrorFLN(PSTR("Sorter cannot home - wait for button press..."));
				int vale = 1;
			...
			Com::printFLN(PSTR("going home to max"));
			position = 0;
			gotoPosition(maxDistance,speed);
			if(!endstopHit()){
				Com::printErrorFLN(PSTR("Sorter cannot home to max"));
				while(1){
					GCode::keepAlive(Processing);
		...
		if(goToCurrent)
			gotoPosition(origPosition,speed);
	}
	void enable()
	{
		HAL::digitalWrite(enablePin, invertEnable);
...
extern void moveMotor(int id, int pos,int speed);
extern void disableAllMotorDrivers();
extern void enableAllMotorDrivers();
